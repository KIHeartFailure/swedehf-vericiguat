...
tab_xls <- tab %>%
  mutate(
    footnote1 = if_else(!is.na(var) & var %in% "shf_quadruple", "*", "")
  ) %>%
  unite(footnote, starts_with("footnote"), sep = "") %>%
  mutate(Variable = paste0(Variable, footnote)) %>%
  select(Variable, pmis, out)
tab_xls <- tab_xls %>%
  add_row(Variable = "*Only registrations after 1 November 2020.")
...
tab <- tab %>%
  mutate(
    footnote1s = if_else(!is.na(var) & var %in% "shf_quadruple", footnote_marker_symbol(1), "")
  ) %>%
  unite(footnote, starts_with("footnote"), sep = "") %>%
  mutate(
    # so no probs
...
default_kable(tab,
  ...
  footnote(
    ...
    symbol = c(
      "Only registrations after 1 November 2020."
    ),
    threeparttable = T
  )
```
